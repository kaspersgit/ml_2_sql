...
def test_config_handling(test_data, test_config):
    ...
    target_col, feature_cols, model_params, pre_params, post_params = config_handling(config, df, logger)
    assert target_col == 'target'
    assert feature_cols == ['col1', 'col2']
    assert model_params == {}
    assert pre_params == {'oot_set': 'false', 'cv_type': 'kfold_cv', 'upsampling': 'false'}
    assert post_params == {'calibration': 'false', 'sql_split': False, 'sql_decimals': 15, 'file_type': 'png'}

def test_checkInputDataHard(test_data, test_config):
    csv_path, df = test_data
    config_path, config = test_config
    
    checkInputDataHard(df, config, None)  # No assertion needed, it should not raise an error

def test_pre_process_kfold(test_data, test_config, test_path):
    csv_path, df = test_data
    config_path, config = test_config

    datasets = pre_process_kfold(test_path, df, 'target', ['col1', 'col2'], model_name='ebm', model_type='classification', logging=logger, pre_params=config['pre_params'], post_params=config['post_params'], random_seed=42)
