...
def apply_model(args):
    ...
    # Set Logger
    logging.basicConfig(format='%(asctime)s %(message)s', filename=destination+'/logging.log', level=logging.DEBUG)
    ...
    if hasattr(model, 'classes_'):
        ...
        if len(model.classes_) == 2:
            ...
            # Threshold dependant
            plotConfusionMatrix(destination, y_true, y_prob, y_pred, file_type='html', data_type='test', logging=logging)
            ...
            plotClassificationCurve(destination, y_true, y_prob, curve_type='roc', data_type='test', logging=logging)

            plotClassificationCurve(destination, y_true, y_prob, curve_type='pr', data_type='test_class1', logging=logging)
            plotClassificationCurve(destination, y_neg, y_prob_neg, curve_type='pr', data_type='test_class0', logging=logging)

            plotClassificationCurve(destination, y_true, y_prob, curve_type='pr', data_type='test_data_class1', logging=logging)
            plotClassificationCurve(destination, y_neg, y_prob_neg, curve_type='pr', data_type='test_data_class0', logging=logging)

            plotCalibrationCurve(destination, y_true, y_prob, data_type='test', logging=logging)

            plotProbabilityDistribution(destination, y_true, y_prob, data_type='test', logging=logging)
        ...
        # If multiclass classification
            ...
            for c in model.classes_:
                ...
                # Threshold independant
                plotClassificationCurve(destination, y_ova, y_prob_ova, curve_type='roc', data_type=f'test_class_{c}', logging=logging)

                plotClassificationCurve(destination, y_ova, y_prob_ova, curve_type='pr', data_type=f'test_class_{c}', logging=logging)

                plotCalibrationCurve(destination, y_ova, y_prob_ova, data_type=f'test_class_{c}', logging=logging)

                plotProbabilityDistribution(destination, y_ova, y_prob_ova, data_type=f'test_class_{c}', logging=logging)
