...
def GetArgs(script, argv):
    ...
    if script == 'main':
      ...
      parser.add_argument("--data_path", type=str, help="Enter path to csv file",
                          nargs='?', const='no_data')
      parser.add_argument("--configuration", type=str, help="Enter path to json file",
                          nargs='?', const='no_config')
      parser.add_argument("--model_name", type=str, help="Enter model type",
                          nargs='?', const='full')
    
    if script == 'test_model':
      parser.add_argument("--model_path", type=str, help="Enter path to model",
                          nargs='?', const='no_name')
      parser.add_argument("--data_path", type=str, help="Enter path to csv file",
                          nargs='?', const='no_data')
      parser.add_argument("--destination_path", type=str, help="Enter path to folder for saving files",
                          nargs='?', const='no_data')
