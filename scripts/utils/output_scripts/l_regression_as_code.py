...
def extract_parameters(model):
    ...
    try:
        # Extract model type
        model_type = 'regression' if model.__class__.__name__ == 'LinearRegression' else 'classification'
...
def format_sql(model_name, model_type, features, coefficients, intercept, split):
    ...
    linear_combination = " + ".join([f"({coef} * {feature})" for coef, feature in zip(coefficients, features)])
    
    # Include the individual score per feature in the SELECT clause
    feature_scores = "\t, ".join([f"({coef} * {feature}) AS {feature}_score\n" for coef, feature in zip(coefficients, features)])
    ...
    if not split:
        ...
        if model_type == 'regression':
            ...
            print(f'\t, 1 / (1 + EXP(-({linear_combination} + {intercept}))) AS probability')
...
def save_model_and_extras(clf, model_name, sql_split, logging):
    ...
    with open('{model_name}/model/lregression_in_sql.sql'.format(model_name=model_name), 'w') as f:
        with redirect_stdout(f):
            model_name = model_name.split('/')[-1]
            format_sql(model_name, model_type, features, coefficients, intercept, sql_split)
